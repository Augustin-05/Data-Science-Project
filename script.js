...
// === ADDITION: Ton graphique CO2/GDP bar chart ===
data.forEach(d => {
  d.Carbon_Footprint = d.Carbon;
  d.GDP_per_Capita = d.GDP;
  d.ratio = d.Carbon && d.GDP ? d.Carbon / d.GDP : 0;
});

const avgRatio = d3.mean(data, d => d.ratio);
const colorScaleRatio = d3.scaleLinear()
  .domain([d3.min(data, d => d.ratio), avgRatio, d3.max(data, d => d.ratio)])
  .range(["green", "orange", "red"]);

const svg = d3.select("#ecochart"),
      marginBar = { top: 30, right: 20, bottom: 120, left: 60 },
      widthBar = +svg.attr("width") - marginBar.left - marginBar.right,
      heightBar = +svg.attr("height") - marginBar.top - marginBar.bottom;
...
const y = d3.scaleLinear()
  .domain([0, d3.max(data, d => d.Carbon_Footprint) * 1.1])
  .range([heightBar, 0]);
...
chart.selectAll(".bar")
  ...
  .attr("y", d => y(d.Carbon_Footprint))
  .attr("height", d => heightBar - y(d.Carbon_Footprint))
  .attr("width", x.bandwidth())
  .attr("fill", d => colorScaleRatio(d.ratio))
